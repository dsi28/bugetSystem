...
switch ($role) {
	case 'ASAB' :
		$query= "SELECT D.ROLE, D.ID, D.FORM, D.TIME_STAMP, A.ACCOUNT, A.ASAB, A.CSAB, A.SG, A.AGSC, A.VPSA FROM DOCS D, APPROVAL A WHERE D.FORM = A.FORM AND D.ID = A.ID AND D.ROLE = A.ROLE AND A.ASAB = 'T' AND A.ACCOUNT = 'T' AND A.SG = 'T' AND A.CSAB = 'T' AND A.AGSC = 'T' AND A.VPSA = 'T'";
		break;
}
try{
...
//					$array2[$count2] = $rows['FORM'];
				...
				<tr>
					...
					<td><?php echo $rows['ASAB']; ?></td>
					<td><?php echo $rows['SG']; ?></td>
					<td><?php echo $rows['CSAB']; ?></td>
					<td><?php echo $rows['AGSC']; ?></td>
					<td><?php echo $rows['VPSA']; ?></td>
					<td><form action="http://lamp.cse.fau.edu/~dizquierdo2014/SNO/php/goView.php" method="post">
