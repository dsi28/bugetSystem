...
$query_check = "SELECT * FROM fauAMP WHERE ROLE='".$ROLE."' AND ID=$DOCID";
...
if($variation>0){
 if($variation<count($POSITION_NUMBER)){
  for($v=$variation; $v<count($POSITION_NUMBER);$v++){
   $query_add="INSERT INTO fauAMP(ID, ROLE, POSITION_NUMBER, POSITION_TITLE, FTE, ANNUAL_RATE, STATE) VALUES($DOCID,
 ...
 for($v=0; $v<count($POSITION_NUMBER);$v++){
  $J = $v+1;
  $query_update = "UPDATE fauAMP SET POSITION_NUMBER='".$POSITION_NUMBER[$v]."', POSITION_TITLE= '".$POSITION_TITLE[$v]."', FTE = '".$FTE[$v]."', ANNUAL_RATE='".$ANNUAL_RATE[$v]."', STATE='".$STATE[$v]."'WHERE ID=$DOCID AND ROLE='".$ROLE."' AND ROWID=$J";
 ...
 for($v=0; $v<count($POSITION_NUMBER); $v++){
  $query2 = "INSERT INTO fauAMP(ID, ROLE, POSITION_NUMBER, POSITION_TITLE, FTE, ANNUAL_RATE, STATE) VALUES($DOCID,
